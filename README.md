...
| **4 — Native terminal** | Real PTY terminal (vendored Apache-2.0 Termux `terminal-emulator`/`terminal-view`, JNI defects fixed) + BusyBox ash/coreutils, session-aware process teardown | **IN PROGRESS** — plan approved (APPROVE WITH CONDITIONS, all 11 conditions adopted: `docs/phase3-terminal-architecture.md` v2 + `docs/phase3-review-response.md`); steps landed: 1 (vendoring @ `3b66f8799635a4dba4a206563048ff0e6792c487`, upstream suite green in CI), 2 (all three JNI defect classes fixed + async-signal-safe fork→exec window + `librustdroidpty` rename + `sendSignalToProcessGroup`; host mock-JNIEnv/ASan harness in CI, every fix regression-pinned — 8/9 checks fail on unfixed upstream code), 3 (UTF-8 split-across-reads test; union discovery with corrected `setsid` semantics pinned by test; `TerminalSessionController` teardown — close-master → grace → freeze → kill → SIGCONT, all lambdas `runCatching`-guarded), 4 (TerminalEnv `xterm-256color` delta over ProcEnv; vendored session reader restructured to poll+wakeup+owned master close; `TerminalCenter` + `TerminalService` FGS + terminal screen with extra-keys row — busybox shell ships with the step-5 bundle), 5 (BusyBox 1.36.1 static-aarch64 build in the decoupled `busybox.yml`: Termux-derived patches retargeted to `@RUSTDROID_PREFIX@`, shell pins + network client/server drops asserted post-oldconfig AND regression-pinned; manifest-v2 bundle `toolchain-1.85.0-aarch64-bb1.36.1` with busybox entry + `Fs`-guarded `sh`/`ash` symlinks; `ToolchainVerifier` busybox/static-ELF/exec/sh-symlink gates; in-app Licenses screen with the GPL source offer; GPL corresponding-source zip on the release) |
...
- **`[77] Problem with the SSL CA cert` / `error setting certificate verify locations`** — history, in three rounds. The first cause was a dangling/missing `CAfile` (fixed by the bundled-CA design, commit `77d6385`). The second theory — the `CApath` channel via `SSL_CERT_DIR` — was **disproven** by run #30's on-device instrumentation: the failure persisted with `CApath: none` while the CAfile was byte-perfect. The **actual root cause (confirmed from openssl-1.1.1w sources against the run #30 error signature `X509_load_cert_crl_file:system lib`)**: `openssl-src` passes `no-stdio` to OpenSSL's Configure for every Android target (a 2016 workaround for openssl-src-rs#13), which compiles `BIO_new_file()` as a stub that always returns NULL — so OpenSSL simply *cannot read any CA file at all*, regardless of path, permissions, or content, while curl's own stdio reads the bundle fine. Fixed in the toolchain build by (a) post-vendor patch `0003` enabling stdio in the vendored `openssl-src` (root fix), and (b) a fallback CA loader in the patched curl that loads the bundle via C stdio + memory BIO if `X509_STORE_load_locations` still fails. Toolchain bundles from run #31 onward carry the fix; the console prints `RUSTDROID-DEBUG: BIO_new_file=<ptr>` (non-NULL = fix active) when diagnosing.
...
## Known limits (v0 terminal)

- **No network applets in the terminal's BusyBox.** Network clients (`wget`, `telnet`, `ftpget`/`ftpput`, `tftp`, `nslookup`, `ping`, `traceroute`, `nc`, `ntpd`, `rdate`, `whois`, `sendmail`, `popmaildir`) and network servers (`telnetd`, `ftpd`, `httpd`, `tftpd`, `udhcpd`, `inetd`, `dnsd`, `udpsvd`, `tcpsvd`) are compiled OUT (`busybox/busybox.config`, asserted in `busybox/build.sh` after `make oldconfig`). Reason: a statically-linked bionic binary has no dynamic NSS/DNS resolution path, so shipped network clients would be broken-by-design at hostname resolution — "wget is broken" reports are worse than no wget. Local network-admin tools (`ifconfig`, `route`, `ip`, `netstat`, `arp`) are kept: they open no sockets and resolve no names. Use the app's crates.io dependency screen for network fetching. Re-enabling a resolver-capable network client is a recorded follow-up (`docs/phase3-terminal-architecture.md` §7.2), not a silent gap.
- **A fully detached escapee survives terminal teardown by design.** A process that called `setsid` AND released the tty is invisible to the union teardown (session members + descendants + tty holders); the plan pins this as correct kernel semantics, not a bug. A `setsid` escapee still HOLDING the tty is caught.
- **Sessions do not survive app death** — no claims otherwise; the FGS keeps the terminal alive while the UI lives, and teardown is best-effort deterministic (quiesce → kill pg → bounded sweeps → SIGCONT survivors).
...
├── .github/workflows/busybox.yml  # decoupled BusyBox 1.36.1 static-aarch64 build (plan §7.2)
├── docs/                          # architecture plans (Phase 4 terminal: phase3-terminal-architecture.md)
├── LICENSES/                      # per-component third-party license texts (terminal-emulator/-view Apache-2.0, busybox GPL-2.0)
├── busybox/                       # BusyBox source pin: build.sh + busybox.config + Termux-derived patches
├── THIRD_PARTY.md                 # third-party inventory: licenses, pins, vendored commits
├── android/                       # the IDE app (Compose, dev.rustdroid.ide)
│   └── app/src/main/assets/       # cacert.pem (Mozilla CA, MPL-2.0) + licenses/ (in-app Licenses screen, plan §7.5)
...
MIT. Design references Termux's public `rust/build.sh` but contains no Termux code or branding (this predates Phase 4; the terminal modules below are Apache-2.0-licensed vendored source with attribution). Rust is MIT/Apache-2.0 (rust-lang.org); the Android NDK is Apache-2.0 + LLVM-variant (Google); the bundled CA store is Mozilla's (`cacert.pem`, MPL-2.0, via curl.se). The full third-party inventory — licenses, pins, vendored commits — lives in `THIRD_PARTY.md`: Phase 4 vendors Termux's Apache-2.0 `terminal-emulator`/`terminal-view` as source (`android/terminal-{emulator,view}/`, pin `3b66f87`) and ships BusyBox 1.36.1 as a separate GPL-2.0 static executable (mere aggregation: `execve`, no linking) with complete corresponding source published as `busybox-1.36.1-src.zip` on the same release as the bundle and the source-offer URL surfaced in-app (Settings → Licenses); see `docs/phase3-terminal-architecture.md` §7.
