...
| **4 — Native terminal** | Real PTY terminal (vendored Apache-2.0 Termux `terminal-emulator`/`terminal-view`, JNI defects fixed) + BusyBox ash/coreutils, session-aware process teardown | **IN PROGRESS** — plan approved (APPROVE WITH CONDITIONS, all 11 conditions adopted: `docs/phase3-terminal-architecture.md` v2 + `docs/phase3-review-response.md`); steps landed: 1 (vendoring both modules @ `3b66f8799635a4dba4a206563048ff0e6792c487`, upstream test suite green in CI), 2 (all three upstream JNI defect classes fixed + async-signal-safe fork→exec window + `librustdroidpty` rename + `sendSignalToProcessGroup`; host-side mock-JNIEnv/ASan harness in CI with every fix regression-pinned — 8/9 checks fail on unfixed upstream code) |
