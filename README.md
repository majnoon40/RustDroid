...
| 2 — App shell | Android app (`dev.rustdroid.ide`) that downloads/bundles the toolchain, provides terminal + editor | **BUILT, on-device first pass done** (see `android/`): install/download/verify loop works on a TECNO-LJ9; first-report bugs fixed in-app (bare-name exec, gate screen contrast/theme, verify-return navigation); debug APK built by the "Android app" workflow |
