...
fun AppRoot(
    ...
    // Crash flight recorder (v0.1.8): if the previous run died from an
    // uncaught exception, CrashRecorder persisted the report — surface
    // it ONCE, in-app, copyable. No external tooling, no files: the user
    // can paste the report straight back into the conversation.
    val crashContext = LocalContext.current
    var crashReport by remember {
        mutableStateOf(dev.rustdroid.ide.runtime.CrashRecorder.lastCrash(crashContext.filesDir))
    }
    if (crashReport != null) {
        val report = crashReport!!
        AlertDialog(
            onDismissRequest = { },
            title = { Text("RustDroid closed unexpectedly") },
            text = {
                Column(
                    Modifier
                        .heightIn(max = 360.dp)
            ...
            confirmButton = {
                TextButton(onClick = {
                    val cm = crashContext.getSystemService(
                        android.content.Context.CLIPBOARD_SERVICE
                    ) as? android.content.ClipboardManager
                    cm?.setPrimaryClip(
                        android.content.ClipData.newPlainText("rustdroid-crash", report)
                    )
                }) { Text("Copy") }
            },
            dismissButton = {
                TextButton(onClick = {
                    dev.rustdroid.ide.runtime.CrashRecorder.clear(crashContext.filesDir)
                    crashReport = null
                }) { Text("Dismiss") }
            },
        )
