...
class RsImportTest {
    ...
    @Test
    fun `ensureImportProject creates a valid cargo bin project without toolchain`() {
        ...
        assertTrue(toml.contains("name = \"imported\""))
        assertTrue(toml.contains("edition = \"2021\""))
        // lives under the projects root, so HomeScreen shows it like any project
        assertEquals(File(tmp.root, "imported"), dir)
    }

    @Test
    fun `ensureImportProject is idempotent`() {
        val dir = repo().ensureImportProject()
        val marker = File(dir, "src/main.rs")
        marker.writeText("// user edits")
        repo().ensureImportProject()
        assertEquals("// user edits", marker.readText())
    }
    ...
    fun `traversal-looking names are sanitized to their basename`() {
        ...
        assertEquals(1, tmp.root.list()!!.count { it == "imported" })
    }
