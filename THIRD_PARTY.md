...
| `terminal-emulator/src/main/java/com/termux/terminal/TerminalSession.java` | — | passes `int[1] ptsDevice` to createSubprocess, stores `mPtsDevice`, exposes `getPtsDevice()` | RustDroid addition (plan §5.2): the session controller's union discovery consumes the pts device number |
